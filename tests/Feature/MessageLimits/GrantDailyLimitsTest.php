...
use App\Actions\GrantDailyLimits;
...
it('is idempotent within current window', function () {
    ...
    MessageLimit::forUser($user)->where('model_type', ModelType::Gpt4oMini->value)->increment('used', 3);
...
it('batches all users via forAll', function () {
    ...
    foreach ($users as $user) {
        expect(MessageLimit::forUser($user)->count())->toBe(2);
    }
});
